...
bounds <- aggregate(olinda_sirgas2000[,"SETOR_ID"], by=list(rep(1, nrow(olinda_sirgas2000))), head, n=1)
...
```{r, echo=TRUE}
use_sf()
writeVECT(bounds, "bounds", v.in.ogr_flags=c("o", "overwrite"))
```

```{r, echo=TRUE}
use_sp()
execGRASS("r.in.gdal", flag=c("o", "overwrite"), input="output/elevation.tif", output="dem")
```

```{r, echo=TRUE}
execGRASS("r.watershed", flag="overwrite", elevation="dem", stream="stream", threshold=1000L, convergence=5L, memory=300L)
execGRASS("r.thin", flag="overwrite", input="stream", output="stream1", iterations=200L)
execGRASS("r.mask", vector="bounds")
execGRASS("r.to.vect", flag="overwrite", input="stream1", output="stream", type="line")
```

```{r, echo=TRUE}
use_sf()
imputed_streams <- readVECT("stream")
```
