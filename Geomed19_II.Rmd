...
```{r, echo=TRUE}
use_sp()
execGRASS("r.in.gdal", flag=c("overwrite"), input="output/elevation.tif", output="dem")
execGRASS("g.region", raster="dem")
```

```{r, echo=TRUE}
execGRASS("r.watershed", flag="overwrite", elevation="dem", stream="stream", threshold=2500L, convergence=5L, memory=300L)
execGRASS("r.thin", flag="overwrite", input="stream", output="stream1", iterations=200L)
```

```{r, echo=TRUE}
use_sf()
writeVECT(bounds, "bounds", v.in.ogr_flags="overwrite")
execGRASS("r.mask", vector="bounds", flag="overwrite")
